...
/** Explicit consent to replay writes when the API offers no idempotency contract. */
export const RETRY_UNSAFE_REQUESTS_FLAG = 'retry-unsafe-requests';
...
export function canReplayRequest(
  ...
  if (retryUnsafeRequests || ['GET', 'HEAD', 'OPTIONS', 'PUT', 'DELETE', 'TRACE'].includes(method.toUpperCase())) return true;
  const name = (idempotencyHeader ?? 'Idempotency-Key').toLowerCase();
  return Object.entries(headers).some(([key, value]) => key.toLowerCase() === name && value.trim().length > 0);
}
