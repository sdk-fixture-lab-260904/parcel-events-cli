...
import { resolveRetryPolicy } from './retries.js';
import { bindRequest, coerceParams, dryRunPlan, type RedactedAuth } from './transport.js';
import { bindBodyLeaves } from './bind-body.js';
import { assembleBody } from './uploads.js';
...
async function bindAndRun(input: {
  ...
  const live = !dryRun && !mock;
  const ready = live ? await materializeOauth(auth.material, io) : { ok: true as const, material: auth.material };
  if (!ready.ok) {
    writeError(io, EXIT_CODES.AUTH, ready.message);
    return EXIT_CODES.AUTH;
  }
  return sendBound({
    ctx,
...
async function sendBound(input: {
  ...
  return runLive({ ctx, request, timeoutMs, maxItems, policy });
}
