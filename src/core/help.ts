...
export const GLOBAL_FLAG_ROWS: readonly { readonly flag: string; readonly note: string }[] = [
  ...
  { flag: '--max-retries <n>', note: 'override the retry budget (0 = no retry; unsafe writes still require opt-in)' },
  { flag: `--${RETRY_UNSAFE_REQUESTS_FLAG}`, note: 'allow retries of writes without an idempotency key (may duplicate effects)' },
